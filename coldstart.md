...
| 2026-08-14 | v1.7.0 | **Fase 7: Customer Onboarding & User Guide Modal** (Fitur 'Panduan Cara Pesan' di Navbar, Modal Interaktif `UserGuideModal`, Banner 4-Step Workflow di Homepage, client-side 4MB file validation & safe 413 payload handling) | Done | Build `next build` PASSED 100% |
| 2026-08-14 | v1.8.0 | **Fase 8: Pemisahan Role Panduan & Fitur Announcement CMS** (Pemisahan `CustomerGuideModal` & `AdminGuideModal`, Tombol Role-based '📖 Panduan Cara Pesan' vs '⚙️ Panduan Admin & CMS', CMS Pengumuman Publik `/api/announcements` & `/api/admin/announcements`) | Done | Build `next build` PASSED 100% |
| 2026-08-14 | v1.9.0 | **Fase 9: Fix Async Client Component & Promise Suspense Errors** (Refactoring `/slots/[id]/book/page.tsx` menjadi murni async Server Component & memindahkan wizard interaktif ke `src/components/booking-wizard.tsx` berlabel `'use client'`) | Done | Build `next build` PASSED 100% |
| 2026-08-14 | v2.0.0 | **Fase 10: Dedicated Public Landing Page & Authenticated Customer View** (High-converting landing page untuk pengunjung belum login pada `/`, Halaman katalog dedicated `/slots`, dan akses katalog langsung untuk pengiklan yang sudah login) | Done | Build `next build` PASSED 100% |
| 2026-08-18 | v2.1.0 | **Fase 11: Fix Permanent Error Pemesanan Serverless Storage Limit** (Mengganti fallback Base64 string raksasa pada `uploadFile` dengan clean URL path ringkas <60 karakter agar tidak pernah melanggar batas kolom VARCHAR PostgreSQL di Supabase Vercel, serta detail pesan kesalahan presisi) | Done | Build `next build` PASSED 100% |
| 2026-08-18 | v2.2.0 | **Fase 12: Fix Smoking Gun `overlappingBookings` Missing `notes` Column Error** (Menambahkan `select: { id: true }` eksplisit pada `prisma.booking.findMany` pengecekan bentrok tanggal di `POST /api/bookings` & fallback query pada `dashboard` & `track` agar pemesanan 100% sukses tanpa kolom `notes`) | Done | Build `next build` PASSED 100% |
| 2026-08-18 | v2.3.0 | **Fase 13: Fix PostgreSQL 25P02 Transaction Aborted Error** (Refactoring fallback transaksi `POST /api/bookings` dengan pembungkusan `prisma.$transaction` dari luar secara mandiri agar retry transaksi baru selalu fresh tanpa pernah memicu status PostgreSQL transaction aborted `25P02`) | Done | Build `next build` PASSED 100% |
| 2026-08-18 | v2.4.0 | **Fase 14: Refactor Arsitektur Upload File (Split Upload Pattern)** (Memisahkan upload file menjadi 3 request terpisah: 1. Upload asset ke `/api/upload`, 2. Upload bukti ke `/api/upload`, 3. Submit booking JSON ringan ke `/api/bookings`. Menghapus seluruh kode handling binary file dari route booking. Menambahkan route baru `/api/upload` untuk upload 1 file sekaligus) | Done | Build `next build` PASSED 32 routes |
